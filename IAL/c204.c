...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	...
	// pokial nie je zasobnik prazdny
	while (!stackEmpty(s)) {

		stackTop(s, &znak);
		// pokial nie je znakom lava zatvorka
		if (znak == '(') {

			// odstranenie z vrcholu zasobnika
			stackPop(s);
			return;
		}

		// ulozi znak do vystupneho retazca
		postExpr[*postLen] = znak;
		(*postLen)++;
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
	...
	if (stackEmpty(s)) {
		...
		// ulozi znak do zasobniku
		stackPush(s, c);
		return;
	}
	else {

		// ak zasobnik nie je prazdny nacita znak z vrcholu
		stackTop(s, &znak);

		// podmienka ak nenarazime na lavu zatvorku
		// kontrolujeme prioritu!
		// ak je znak (* alebo /) c nebude mat vyssiu prioritu
		// ak je c (+ alebo -) nebude v zasobniku operator s vyssou prioritou
		if ((znak != '(') && (znak == '/' || znak == '*' || c == '+' || c == '-')) {

			// vlozime operator do retazca
			postExpr[*postLen] = znak;
			(*postLen)++;

			// odstranime operator zo zasobniku
			stackPop(s);

			// rekurzivne zavolame funkciu
...
char* infix2postfix (const char* infExpr) {
	...
	// alokujeme pamat pre vystupne pole znakov
	postExpr = malloc(sizeof((char*) MAX_LEN));

	// ak sa alokacia podarila
	if (postExpr != NULL) {
		...
		if (s != NULL) {
			...
			while (i < MAX_LEN) {
				...
				if (infExpr[i] ==  '(') {
					...
					// zacnem odoberat zo zasobnika a pridavat na koniec vystupneho retazca az po '('
					untilLeftPar(s, postExpr, &postLen);
				}
				else if (infExpr[i] == '+' || infExpr[i] == '-' || infExpr[i] == '*' || infExpr[i] == '/') {
					
					// spracujeme operator cez funkciu
					doOperation(s, infExpr[i], postExpr, &postLen);
				}
				else if (infExpr[i] == '=') {

					// cyklus na postupne odstranovanie prvkov zo zasobnika a vkladanie na koniec vystupneho retazca
					while (!stackEmpty(s)) {
						stackTop(s, &postExpr[postLen++]);
						stackPop(s);
					}

					// na koniec pridam '=' a ukoncovaci znak
					postExpr[postLen++] = '=';
					postExpr[postLen] = '\0';
				}
				else {
					
					// vlozi znak do vystupneho retazca
			...
			// uvolnime alokovanu pamat pre zasobnik
			free(s);
			return postExpr;
		}
		else {

			// uvolnime alokovanu pamat pre vystupny retazec
			free(postExpr);
			return NULL;
		}
	}
