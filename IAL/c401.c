...
**/
	...
	if ((*RootPtr) != NULL) {
		...
		// najviac vpravo
			...
			// vlozim hodnoty z najpravsieho uzlu
			PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
			PtrReplaced->Key = (*RootPtr)->Key;

			// spojim s lavym podstromom
...
**/	
	...
	if (*RootPtr != NULL) {
		...
		// ak je kluc vacsi idem doprava
		if (node->Key < K) {
			if (node->RPtr != NULL) {

				BSTDelete(&node->RPtr, K);
				
			}
		}

		// ak je kluc mensi idem vlavo
		else if (node->Key > K) {
			if (node->LPtr != NULL) {

				BSTDelete(&node->LPtr, K);
			
			}
		}
		else {

			// ak pravy podstrom neexistuje
			if (node->RPtr == NULL) {
